//! Gemma provider configuration loaded from environment variables.
...
impl GemmaConfig {
    pub(super) fn from_env() -> Self {
        Self::from_values(
            env::var("GEMMA_BASE_URL").ok().as_deref(),
            env::var("GEMMA_MODEL").ok().as_deref(),
            env::var("GEMMA_API_KEY").ok().as_deref(),
            env::var("GEMMA_IMAGE_DETAIL").ok().as_deref(),
        )
    }
...
mod tests {
    ...
    fn default_config_targets_local_lm_studio_responses_api() {
        ...
        assert_eq!(config.model, "google/gemma-4-26b-a4b");
    ...
    fn config_trims_overrides_and_removes_trailing_base_url_slash() {
        let config = GemmaConfig::from_values(
        ...
        assert_eq!(config.model, "custom/gemma");
