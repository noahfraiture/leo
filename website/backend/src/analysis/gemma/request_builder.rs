//! Gemma Responses API request construction for frame chunks and summaries.

use base64::{Engine, engine::general_purpose::STANDARD};
use serde_json::{Value, json};

use crate::{
    analysis::{chunking::FrameChunk, prompts::gemma as prompts},
    media::VideoFrame,
};
...
pub(super) struct GemmaChunkRequest<'a> {
    pub config: &'a GemmaConfig,
    pub prompt: &'a str,
...
pub(super) enum GemmaImageInput<'a> {
    Base64DataUrl {
...
pub(super) fn gemma_frame_payload_bytes(frame: &VideoFrame) -> usize {
    gemma_frame_image_input(frame).estimated_payload_bytes()
}

pub(super) fn generate_response_request(request: GemmaChunkRequest<'_>) -> Value {
    ...
    for (index, frame) in request.chunk.frames.iter().enumerate() {
        ...
        content.push(gemma_frame_image_input(frame).to_json(&request.config.image_detail));
    }
...
fn gemma_frame_image_input(frame: &VideoFrame) -> GemmaImageInput<'_> {
    GemmaImageInput::Base64DataUrl {
...
mod tests {
    ...
    use super::{
        GemmaChunkRequest, GemmaImageInput, gemma_frame_payload_bytes, generate_response_request,
        summarize_chunks_request,
    };
    use crate::analysis::gemma::config::GemmaConfig;

    #[test]
    fn generate_response_request_uses_lm_studio_model_and_frame_inputs() {
        let config = GemmaConfig::from_values(None, None, None, None);
    ...
    fn image_input_estimates_base64_payload_instead_of_raw_bytes() {
        let bytes = [b'x'; 3];
        let input = GemmaImageInput::Base64DataUrl {
            mime_type: "image/jpeg",
            bytes: &bytes,
        };
    ...
    #[test]
    fn gemma_frame_payload_bytes_counts_image_payload() {
        ...
        assert!(gemma_frame_payload_bytes(&frame) > frame.bytes.len());
    }
}
