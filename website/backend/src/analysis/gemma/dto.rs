//! Gemma response DTOs and text extraction.

use serde::Deserialize;

#[derive(Deserialize)]
pub(super) struct GemmaResponse {
    #[serde(default)]
    output_text: Option<String>,
    #[serde(default)]
    output: Vec<GemmaOutputItem>,
}
...
#[derive(Deserialize)]
struct GemmaOutputItem {
    #[serde(default)]
    content: Vec<GemmaContent>,
}
...
mod tests {
    ...
    use super::GemmaResponse;

    #[test]
    fn response_text_prefers_output_text() {
        let response: GemmaResponse = serde_json::from_value(json!({
            "output_text": "summary"
        }))
