...
fn persist_checkpoint(directory: &Path, checkpoint: &AnalysisCheckpoint) -> Vec<u8> {
    let mut contents = serde_json::to_vec_pretty(checkpoint).expect("checkpoint should serialize");
    contents.push(b'\n');
    fs::write(directory.join("analysis.json"), &contents).expect("checkpoint should be written");
    contents
}
...
async fn failed_save_never_emits_an_unsaved_response() {
    ...
    let result = analyze_with_mock(
        ...
        |checkpoint| {
            ...
            if snapshots.len() == 1 {
                fs::remove_file(&checkpoint_path).expect("initial checkpoint should be removed");
